...
#' 
#' @noRd
app_spaghettiplot <- function(data, yvar, time, value = NULL, color = NULL, facet = NULL) {
  if (yvar %in% colnames(data)) {
    ...
    if(is.null(color) || color == ''){
      ...
      p <- ggplot2::ggplot(data) + 
        ggplot2::aes_string(x = time, y = yvar, color = color, group = "USUBJID") +
        ggplot2::ylab(best_lab(data, yvar)) +
        ggplot2::xlab(best_lab(data, time))
    }
    ...
    if(is.null(color) || color == ''){
      ...
      p <- d %>%
        ggplot2::ggplot() +
        ggplot2::aes_string(x = time, y = value, group = "USUBJID", color = color)  +
        ggplot2::ylab(
          glue::glue("{var_label} ({best_lab(d, value)})")
        ) +
        ggplot2::xlab(best_lab(data, time))
